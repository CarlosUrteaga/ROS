...
void cLaneDetectionFu::ProcessInput(const sensor_msgs::Image::ConstPtr& msg)
{
   ...
   //     cv::waitKey(1);
    ...
    #ifdef PAINT_OUTPUT
    cv::Mat transformedImagePaintableJCTest;
        transformedImagePaintableJCTest = transformedImage.clone();
    //agregar color a la imagen
         cv::cvtColor(transformedImagePaintableJCTest,transformedImagePaintableJCTest,CV_GRAY2BGR);
//pinta lineas
        cv::Point2d p11l(25,minYPolyRoi);
        cv::Point2d p21l(25,maxYRoi-1);
        cv::line(transformedImagePaintableJCTest,p11l,p21l,cv::Scalar(0,255,255));

    cv::imshow("Cut IPmapped image", transformedImagePaintableJCTest);
    cv::waitKey(1);
...
//dibuja las lineas
        ...
        cv::line(transformedImagePaintable,p1l,p2l,cv::Scalar(0,255,255));
    ...
    #endif
    #endif    //---------------------- END DEBUG OUTPUT GROUPED LANE MARKINGS ------------------------------//
